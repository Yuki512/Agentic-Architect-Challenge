...
if not exist "%SCRIPT%" (
  echo Part 2 server script was not found:
  echo %SCRIPT%
  pause
  exit /b 1
)
...
if /I "%~1"=="--check" (
  echo Part 2 launcher check passed.
...
if errorlevel 1 (
  start "Part 2 Server - close this window to stop" /D "%PART_DIR%" "%ComSpec%" /k ""%PYTHON_EXE%" "%SCRIPT%""
  call :wait_for_server
  if errorlevel 1 (
    echo Part 2 did not become ready within 12 seconds.
    echo Check the Part 2 Server window for the error.
    pause
    exit /b 1
  )
)
...
if exist "%USERPROFILE%\.cache\codex-runtimes\codex-primary-runtime\dependencies\python\python.exe" (
  "%USERPROFILE%\.cache\codex-runtimes\codex-primary-runtime\dependencies\python\python.exe" -c "import pdfplumber" >nul 2>&1
  if not errorlevel 1 set "PYTHON_EXE=%USERPROFILE%\.cache\codex-runtimes\codex-primary-runtime\dependencies\python\python.exe"
)

for /f "delims=" %%P in ('python -c "import pdfplumber, sys; print(sys.executable)" 2^>nul') do (
  if not defined PYTHON_EXE set "PYTHON_EXE=%%P"
)

if not defined PYTHON_EXE (
  for /f "delims=" %%P in ('py -3 -c "import pdfplumber, sys; print(sys.executable)" 2^>nul') do (
    if not defined PYTHON_EXE set "PYTHON_EXE=%%P"
  )
)
exit /b 0
